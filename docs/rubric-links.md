...
!!! warning "Honest about gaps, not just links"
    Where a criterion doesn't have real evidence yet, that's stated plainly with a ⚠️ below. Sprint 1 is complete with most criteria having solid evidence. Sprint 2's user-feedback round is complete (7 responses, findings and traceability documented); the Sprint 3 round — interviews, hands-on testing, a second survey wave — is still pending. The remaining open gap is **Improvement** (Milestone 3 — the first before/after comparison will come from re-running the feedback survey after Sprint 3 changes). **Performance** (Milestones 3–4) is no longer a documentation gap: [Performance](design/performance.md) records the caching strategy, query consolidation and indexes with measured before/after query counts, alongside the running system itself.
...
| User Experience | App | 5% | [UI Overview](design/wireframes.md), live at [sportsanalytics.pages.dev](https://sportsanalytics.pages.dev/) — 13 routes, navbar navigation, court view visualisation |
