...
- **Model accuracy shown, not just asserted** — ✅ built (PR #94). `GET /v1/analytics/model-accuracy` publishes the model's real accuracy against an **always-pick-home baseline**, because an accuracy figure without a baseline isn't a meaningful number. As of 2026-09-11, over 231 completed games: **64.1% accuracy** against a **58.9%** home baseline, Brier score **0.2163**. The endpoint also reports how many *forward* predictions exist (currently **0**), so a backtest is never presented as a track record, and marks calibration bands with too few games as "n too small" rather than printing a percentage that looks like a fact.
