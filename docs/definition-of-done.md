...
2. **Tests are written and passing in CI** — unit tests at minimum; integration or E2E tests where the change touches an API route or a critical user journey (auth, import, optimiser run). CI runs both API and web test suites against a real Postgres with no tolerance flags — a failing test fails the pipeline. See [Testing](testing.md) and [CI/CD Pipeline](ci-cd.md).
