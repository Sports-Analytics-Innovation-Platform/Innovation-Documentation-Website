...
!!! danger "CI/CD host — reopened, needs a final check"
    Two rounds of this doc have now landed on opposite answers: an earlier pass said Gitea, a later one said "confirmed Gitea" after asking around, and a third (this one) says GitLab based on inspecting the file directly. The evidence for **GitLab**: `.gitlab-ci.yml` is GitLab's exact config filename and uses GitLab-only syntax (`$CI_COMMIT_BRANCH`, `$CI_DEFAULT_BRANCH`, a `pages` job requiring GitLab Pages) — Gitea Actions reads `.gitea/workflows/*.yml` in GitHub-Actions syntax and would not execute this file at all. `docs/ai-usage.md`'s 2026-08-07 entry (written the day the pipeline was built) also says "GitLab CI pipeline... published to GitLab Pages," and `scripts/post-mr-coverage-comment.mjs` uses "MR," GitLab's term. If the pipeline is actually running — and the coverage badge setup suggests it is — something has to be parsing `.gitlab-ci.yml`, and only GitLab does that. **Before this goes in the group report**: log into `sdp.ms.wits.ac.za` and check whether the UI is GitLab- or Gitea-branded — that settles it in five seconds without relying on inference either way.
